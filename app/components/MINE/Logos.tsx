...
export default function Logos({ ArrayOfText }: { ArrayOfText?: string[] }) {
    ...
    return (
        <div ref={reference} className="flex flex-row items-center justify-start gap-20 overflow-hidden w-full bg-black">
            {AltArray.map((logo, index) => (
                <div key={index} className="flex flex-row items-center lg:gap-40 gap-30">
                    <figure className="relative flex flex-col gap-2 lg:py-7 py-5">
                        ...
                        <q className="text-white text-nowrap text-center lg:text-3xl text-xl">
                            {logo}
                        </q>
                    </figure>
                </div>
